...
11. **Specificity of repeating blocks** — Each `+`/`*` repetition block (arg-level or fact-level) contributes 1 point to the pattern's specificity, plus the specificity of its inner args/facts. `?` optionals contribute 0. So a pattern with more repeating blocks is more specific than one with fewer (e.g. `sentence $( $a )+ . $( $b )+` scores higher than `sentence $( $a )+ .`).
