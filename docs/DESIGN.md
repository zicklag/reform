...
9. **Lazy repetitions** — Arg-level `+`/`*` repetitions are lazy: they match as few iterations as possible. When a single fact admits several full-consumption matches, the one that peels off the fewest arguments from the leftmost repetition wins; only that binding fires. Optional `?` blocks remain greedy (prefer one iteration, fall back to zero). Fact-level repetitions still collect all matching facts.
...
11. **Specificity of repeating blocks** — A `+` (one-or-more) repetition block contributes 1 point for the block itself plus its inner args/facts; `*` (zero-or-more) and `?` (optional) may match zero, so they contribute 0 for the block (only their inner args/facts count, and `?` contributes 0 entirely). This ranks a structured rule with literal constraints above a catch-all like `sentence $( $arg )*`, while still ranking `sentence $( $a )+ . $( $b )+` above `sentence $( $a )+ .` (more required `+` blocks = more specific).
